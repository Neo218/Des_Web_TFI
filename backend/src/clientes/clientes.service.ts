...
export class ClientesService {
  ...
  async create(createClienteDto: CreateClienteDto, usuario?: UsuarioAutenticado): Promise<{ id: number }> {
    ...
    await this.historialService.registrarCambio({
      ...
      accion: 'CREACION',
      usuario,
      detalle: { despues: { id: saved.id, nombre: saved.nombre, estado: saved.estado, direccion: saved.direccion } },
    });
    return { id: saved.id };
  }
  ...
  async findAll(): Promise<ListClienteDto[]> {
    ...
    return clientes.map(c => ({ id: c.id, nombre: c.nombre, telefono: c.telefono, email: c.email, direccion: c.direccion, estado: c.estado }));
  }
  ...
  async findOne(id: number): Promise<ListClienteDto> {
    ...
    return { id: cliente.id, nombre: cliente.nombre, telefono: cliente.telefono, email: cliente.email, direccion: cliente.direccion, estado: cliente.estado };
  }

  async update(id: number, updateClienteDto: UpdateClienteDto, usuario?: UsuarioAutenticado): Promise<ListClienteDto> {
    const cliente = await this._findOneEntity(id);
    const antes = { id: cliente.id, nombre: cliente.nombre, telefono: cliente.telefono, email: cliente.email, direccion: cliente.direccion, estado: cliente.estado };
    ...
    await this.historialService.registrarCambio({
      ...
      accion: 'MODIFICACION',
      usuario,
      detalle: { antes, despues: { id: saved.id, nombre: saved.nombre, telefono: saved.telefono, email: saved.email, direccion: saved.direccion, estado: saved.estado } },
    });
    return { id: saved.id, nombre: saved.nombre, telefono: saved.telefono, email: saved.email, estado: saved.estado };
  }
  ...
  async remove(id: number, usuario?: UsuarioAutenticado): Promise<ListClienteDto> {
    ...
    await this.historialService.registrarCambio({
      ...
      detalle: {
        antes,
        despues: { id: saved.id, nombre: saved.nombre, telefono: saved.telefono, email: saved.email, direccion: saved.direccion, estado: saved.estado },
      },
    });
